...
def send_payment_webhook(salon_id):
    ...
    webhook_url = "https://app.alteg.io/marketplace/partner/payment"
    ...
    data = {
        ...
        "currency_iso": "USD",             # валютa платежа
    ...
    try:
        # Выполняем POST-запрос "id":45112
...
def refund_request(payment_id):
    url = f"https://app.alteg.io/marketplace/partner/payment/refund/{payment_id}"  

    # Заголовки авторизации b'{"success":false,"data":null,"meta":{"message":"To use the API, specify the Accept: application/vnd.api.v2+json header in the request"}}'
    headers  = {
